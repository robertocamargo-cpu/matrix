...
import { UserCheck, Award, Phone, Mail, Link as LinkIcon, Layers, ShieldCheck, AlertTriangle, Trash2, ExternalLink } from 'lucide-react';
...
export const DecisionMakersGrid: React.FC<DecisionMakersGridProps> = ({
  ...
  // Importance logic (Proprietário 5, Diretor 4, Compras 1, Operações 2, Facilities 3)
  const getImportanceScore = (ranking: number) => {
    switch (ranking) {
      case 5: return 1; // Proprietário / CEO (Máxima Importância)
      case 4: return 2; // Diretor (Alta Importância)
      case 1: return 3; // Compras (Alta/Média Importância)
      case 2: return 4; // Operações (Média Importância)
      case 3: return 5; // Facilities (Baixa/Focada Importância)
      default: return 6;
    }
  };

  // Sort primarily by role importance score (lower is more important), with active verification status as tie-breaker
  const rankedDMs = [...consolidatedDMs].sort((a, b) => {
    const scoreA = getImportanceScore(a.ranking);
    const scoreB = getImportanceScore(b.ranking);
    
    if (scoreA !== scoreB) {
      return scoreA - scoreB;
    }
    
    // Status fallback tie-breaker (Confirmado first, then active/pending, then other places/rejected)
    const aConf = a.status === 'Confirmado' ? 1 : a.status === 'Trabalha em outro lugar' ? 3 : a.status === 'Rejeitado' ? 4 : 2;
    const bConf = b.status === 'Confirmado' ? 1 : b.status === 'Trabalha em outro lugar' ? 3 : b.status === 'Rejeitado' ? 4 : 2;
    return aConf - bConf;
  });
  ...
  const getRankBadgeAndMeta = (ranking: number) => {
    switch (ranking) {
      ...
      case 3:
        return { label: 'Facilities (Técnico)', color: 'bg-indigo-100 text-indigo-800 border-indigo-300' };
      case 4:
    ...
    <div className="bg-white rounded-xl border border-slate-100 shadow-sm p-5 space-y-4">
      <div className="flex justify-between items-center border-b border-slate-100 pb-3">
        ...
        <span className="text-[10px] text-slate-400 font-mono italic">
          Ordenação de Relevância • Controle de Vínculo Ativo
        </span>
      </div>
