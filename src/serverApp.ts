...
function buildResponseSchema(
  ...
  const calculateLuxuryProfileScore = () => {
    ...
    let score = 45; // Baseline floor for B2B leads
    const matchingFactors: string[] = [
      'Empresa B2B ativamente qualificada para prospeção (+45 pts)'
    ];

    // 1. High-ticket & Vertical keyword density
    ...
    if (kwCount > 0) {
      ...
      score += kwBoost;
      matchingFactors.push(`Palavras-chave de alto padrão identificadas (${kwCount} termos) (+${kwBoost} pts)`);
    }

    // 2. Elite prime locations
    const eliteCities = ['são paulo', 'rio de janeiro', 'curitiba', 'porto alegre', 'belo horizonte', 'florid', 'floripa', 'balneario', 'balneário', 'jardins', 'alphaville', 'leblon', 'ipanema', 'itaim', 'faria lima'];
    let cityFound = false;
    eliteCities.forEach(city => {
      if (!cityFound && textToAnalyze.includes(city)) {
        cityFound = true;
        score += 15;
        matchingFactors.push(`Localização estratégica em hub de alto consumo (${city.toUpperCase()}) (+15 pts)`);
      }
    });
    ...
    if (textToAnalyze.includes("hotel") || textToAnalyze.includes("resort") || textToAnalyze.includes("motel") || textToAnalyze.includes("pousada")) {
      score += 20;
      matchingFactors.push('Setor de Hospitalidade Premium / Hotelaria / Motéis (+20 pts)');
    } else if (textToAnalyze.includes("restaurante") || textToAnalyze.includes("gastronomia") || textToAnalyze.includes("bistrô") || textToAnalyze.includes("michelin")) {
      score += 20;
      matchingFactors.push('Setor de Restaurantes de Luxo / Fine Dining (+20 pts)');
    } else if (textToAnalyze.includes("hospital") || textToAnalyze.includes("clínica") || textToAnalyze.includes("clinica") || textToAnalyze.includes("spa")) {
      score += 20;
      matchingFactors.push('Setor de Saúde VIP, Clínicas de Elite e SPAs (+20 pts)');
      ...
      score += 20;
      matchingFactors.push('Setor Corporativo Sênior / Advocacia / Financeiro (+20 pts)');
    }
...
0.1.3 BUSCA OBRIGATÓRIA DOS CARGOS FOCO DA MATRIZ NEVINE (FORÇAR ENCONTRÁ-LOS CONFORME O SEGMENTO DO CLIENTE):
  Ao analisar a empresa, você DEVE buscar ativamente os profissionais que ocupam os seguintes cargos estratégicos de acordo com o segmento:
  ...
  * SE FOR HOSPITAL / CLÍNICA / MATERNIDADE / SAÚDE: Busque ativamente por: "Nutricionista Responsável Técnica (RT)", "Nutricionista Chefe", "Gestor de SND (Serviço de Nutrição e Dietética)", "Coordenador de SCIH (Controle de Infecção)", "Gestor de Hotelaria Hospitalar", "Gerente de Suprimentos Hospitalares".
  * SE FOR RESTAURANTE / BISTRÔ / GASTRONOMIA / FINE DINING: Busque ativamente por: "Maître d'Hôtel", "Chef Executivo", "Sommelier", "Gerente de Operações / Salão", "Gerente de Compras".
  * SE FOR MOTEL DE LUXO / BOUTIQUE: Busque ativamente por: "Gerente Geral de Motel", "Governanta / Supervisora de Governança", "Diretor / Proprietário".
  Atribua a hierarquia correta a cada um (Ranking 5: Proprietário/CEO/Sócio; Ranking 4: Diretores; Ranking 3: Gerentes; Ranking 2: Coordenadores/Nutricionistas RT/Supervisores/Sommelier/Maître; Ranking 1: Compras Operacional/Técnicos).

0.1.4 PESQUISA REAL DE VAGAS EM ABERTO E EXPANSÃO:
  Pesquise se a empresa possui vagas de emprego ativas divulgadas em sites de carreiras, Gupy, LinkedIn Jobs, Catho, Glassdoor ou Trabalhe Conosco. Retorne as vagas reais encontradas no campo "vagas_abertas" de "newDiscoveries" com títulos claros dos cargos abertos (ex: "Nutricionista Clínico", "Comprador de Suprimentos", "Governanta", "Camareira", "Sommelier") e a URL da vaga em sourceUrl.
